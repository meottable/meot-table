...
const draft=()=>({requestId:crypto.randomUUID(),name:'테스트 고객',business:'고깃집',rating:5,body:'서버 검증용이며 실제 게시하지 않는 후기입니다.',consent:true,photos:[{data:'data:image/jpeg;base64,/9j/2Q=='}]});
...
test('bad input and photos rejected before storage',()=>{const n=root.children.length;for(const patch of [{rating:6},{body:'짧음'},{consent:false},{name:' '},{website:'spam'},{photos:[{data:'data:image/svg+xml;base64,PHN2Zz4='}]},{photos:Array(4).fill({data:'data:image/jpeg;base64,/9j/2Q=='})}])assert.throws(()=>context.submitReview({...draft(),...patch}));assert.equal(root.children.length,n);});
